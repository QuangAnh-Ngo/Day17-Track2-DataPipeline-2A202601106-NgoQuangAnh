...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or trim({{ col }}) = '' then 'priority bị khuyết (NULL hoặc rỗng)'
        when lower(trim({{ col }})) in ('0', '5', '-1') then 'priority là số ngoài khoảng contract (chỉ nhận 1..4)'
        else 'priority không quy đổi được (nhãn không hợp lệ: ' || coalesce({{ col }}, 'NULL') || ')'
    end
{% endmacro %}
